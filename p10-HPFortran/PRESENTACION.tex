...
\begin{frame}[fragile]
  \frametitle{¿Qué es?}
  \emph{High Performance Fortran} es una extensión de \emph{Fortran 90} que
  añade directivas de distribución de datos. Extiende el paralelismo a nivel de
  datos, permitiendo la distribución de estos datos entre multiples
  procesadores.
\end{frame}

\begin{frame}[fragile]
  \frametitle{Historia.}
  La primera versión de \emph{HPF} se publico den 1993 por el \emph{High
...
\begin{frame}[fragile]
  \frametitle{Directivas de distribución de datos.}
  La directiva \texttt{DISTRIBUTE} especifica la partición de lo datos entre
  los procesadores. \\
  La directiva \texttt{ALIGN} fuerza a que los datos correspondientes de dos o
  más arrays esten almacenados en un mismo procesador. \\
  La directiva \texttt{PROCESSORS} especifica la configuración de los
...
\begin{frame}[fragile]
  \frametitle{Directiva FORALL.}
    ...
    \end{center}
    donde \texttt{assignment} es una asignación aritmética o de punteros y
